...
copy_error_message = "Copying of pages is disabled, create a new page instead."
...
def prevent_direct_publish(
    ...
    if settings.DISABLE_DIRECT_PUBLISH:
...
def prevent_bulk_direct_publish(
    ...
    if settings.DISABLE_DIRECT_PUBLISH and action_type == "publish":
        messages.error(request, publish_error_message)
        return redirect("wagtailadmin_home")


@hooks.register("before_copy_page")
def prevent_page_copy(
    request: HttpRequest, page: Page, **kwargs: dict[str, Any]
) -> None | HttpResponse:
    """Prevent page copying."""

    # this hook is called before a page is copied.
    if settings.DISABLE_DIRECT_PUBLISH:
        messages.error(request, copy_error_message)
        return redirect("wagtailadmin_home")
...
def remove_not_allowed_actions(
    ...
    if (
        settings.DISABLE_SELF_APPROVAL
        and user
