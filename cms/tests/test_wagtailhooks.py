...
class TestPreventPageCopy(TestCase):
    """Test that page copying can be prevented."""
    ...
    @override_settings(DISABLE_DIRECT_PUBLISH=True)
    def test_prevents_page_copy_when_disabled(self):
        """Test that page copying is prevented when disabled."""
        response = prevent_page_copy(self.request, self.page)
        stored_messages = list(messages.get_messages(self.request))

        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse("wagtailadmin_home"))
        self.assertEqual(len(stored_messages), 1)
        self.assertEqual(stored_messages[0].message.strip(), copy_error_message)

    @override_settings(DISABLE_DIRECT_PUBLISH=False)
    def test_allows_page_copy_when_enabled(self):
        """Test that page copying is allowed when direct publishing is enabled."""
        response = prevent_page_copy(self.request, self.page)
