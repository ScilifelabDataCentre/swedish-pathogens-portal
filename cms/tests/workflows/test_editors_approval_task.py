...
class TestEditorsApprovalTask(TestCase):
    ...
    @override_settings(DISABLE_SELF_APPROVAL=True)
    ...
    @override_settings(DISABLE_SELF_APPROVAL=False)
    ...
    def test_on_action_allows_self_approval_when_enabled(self):
        ...
        with patch.object(AbstractGroupApprovalTask, "on_action") as parent_on_action:
            self.task.on_action(task_state, self.user, "approve")
